...
class Forward_Model(torch.nn.Module):
    def __init__(
        ...
        if self.blur_type == "symmetric":
            self.w_init = np.repeat(
                np.array(w_init[0])[np.newaxis], self.num_ims, axis=0
            )
            self.w_blur = torch.nn.Parameter(
                torch.tensor(self.w_init, dtype=torch.float32, device=self.cuda_device)
            )
        else:
    ...
    def Hfor(self, x, i):
        if self.blur_type == "symmetric":
            psf_sharp = torch.exp(
                -((self.X / self.w_blur[i]) ** 2 + (self.Y / self.w_blur[i]) ** 2)
            )
        else:
            psf_sharp = torch.exp(
                -((self.X / self.w_blur[i, 0]) ** 2 + (self.Y / self.w_blur[i, 1]) ** 2)
            )

        psf_sharp = psf_sharp / torch.linalg.norm(psf_sharp)
        h_complex = pad_zeros_torch(
            self, torch.complex(psf_sharp, torch.zeros_like(psf_sharp)).unsqueeze(0)
        )
    ...
    def forward(self, in_image):
        ...
        for i in range(0, self.num_ims):
            output = torch.sum(
                self.shutter_var
                * crop_forward(self, self.Hfor(my_pad(self, in_image), i)),
                1,
            )
...
class MyEnsemble(nn.Module):
    ...
    def forward(
        self, x, spec_dim=2, sim=True
    ):  # spectral channel needs to be padded up to power of 2
        self.output1 = self.model1(x, sim)[0].unsqueeze(0)
