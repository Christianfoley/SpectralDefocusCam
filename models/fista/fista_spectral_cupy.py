...
if device == "GPU":
    import cupy as np
    import models.fista.tv_approx_haar_cp as tv
...
class fista_spectral_numpy:
    def __init__(self, h, mask):
        ...
        self.H = np.expand_dims(
            np.fft.fft2((np.fft.ifftshift(self.pad(h), axes=(0, 1))), axes=(0, 1)), -1
        )
        ...
        self.tau = 0.5  # Native sparsity tuning parameter
    ...
    def loss(self, x, err):
        ...
        if self.prox_method == "native":
            l = np.linalg.norm(err) ** 2 + 2 * self.tv_lambda / self.L * np.linalg.norm(
                x.ravel(), 1
            )
        if self.prox_method == "non-neg":
    ...
    def run(self, inputs):
        ...
        for i in range(0, self.iters):
            ...
            if self.show_recon_progress == True and i % self.print_every == 0:
                ...
                plt.subplot(1, 2, 1), plt.imshow(fc_img / numpy.max(fc_img))
